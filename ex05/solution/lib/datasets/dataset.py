...
class Dataset(torch.utils.data.Dataset, metaclass=abc.ABCMeta):
    ...
    @classmethod
    def init_as_loader(cls, batch_size=1, shuffle=False, num_workers=0, pin_memory=False, drop_last=False, worker_init_fn=None, **kwargs):
        dataset = cls(to_torch=True, **kwargs)
        return dataset.get_loader(batch_size=batch_size, shuffle=shuffle, pin_memory=pin_memory, num_workers=num_workers, drop_last=drop_last, worker_init_fn=worker_init_fn)

    def get_loader(self, batch_size=1, shuffle=False, num_workers=0, pin_memory=False, drop_last=False, worker_init_fn=None):
        return torch.utils.data.DataLoader(self, batch_size=batch_size, shuffle=shuffle, pin_memory=pin_memory, num_workers=num_workers, drop_last=drop_last, worker_init_fn=worker_init_fn)
