...
def warp(x, offset=None, grid=None, padding_mode='border'):  # based on PWC-Net Github Repo
    """
    warp an image/tensor according to an offset (optical flow), or according to given grid locations.
    x: [N, C, H, W] (im2)
    ...
    Output: [N, C, h_out, w_out] ; [N, 1, h_out, w_out] Sampled points from x and sampling masks.
    """
...
def shift_multi(x, offsets, padding_mode='zeros'):
    ...
    :param x: Feature map. NCHW.
    :param offsets: Shift offsets. S2.
    :return: Shifted feature maps and masks. NSCHW and NSHW.
    """
...
def shift(x, offset, padding_mode='zeros'):
    """
    Shifts features x with a given integer offset.

    :param x: Feature map. NCHW.
    :param offset: Shift offset. 2.
    :return: Shifted feature map and mask. NCHW and N1HW.
    """
