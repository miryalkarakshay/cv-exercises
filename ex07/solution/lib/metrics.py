...
def epe(gt, pred, weight=None):
    ...
    epe = epe * weight if weight is not None else epe
    avg_epe = epe.mean()
    return avg_epe
...
def pointwise_epe(gt, pred, weight=None):
    ...
    pointwise_epe *= weight if weight is not None else pointwise_epe
...
def photometric_loss(image_1, image_2, pred_flow, weight=None, alpha=0.5, eps=1e-3):
    ...
    pointwise_photo_error = torch.sum(charbonnierLoss(image_diff, alpha, eps), dim=1, keepdim=True) * warping_mask
...
def smoothness_loss(pred_flow, weight=None, alpha=0.3, eps=1e-3):
    ...
    pointwise_smoothness_loss = torch.sum(torch.sum(charbonnierLoss(flow_diffs, alpha, eps), dim=2), dim=1, keepdim=True)  # N1HW
...
def compute_disp_metrics(sample, model_output):
    ...
    if 'pred_disp' in model_output:
        pred_disp = model_output['pred_disp']
    else:
        pred_disp = model_output['pred_flow'][:,0:1,:,:]
