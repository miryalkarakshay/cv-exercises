...
class UniformTorchCorr(nn.Module):
    ...
    def forward(self, feat_ref, feat_src, offsets):
        ...
        pad_top, pad_bot = max(0, -1*torch.min(offsetsy).item()), max(0, torch.max(offsetsy).item())
        pad_size = (pad_l, pad_r, pad_top, pad_bot)
        pad_fct = nn.ConstantPad2d(pad_size, 0) if self.padding_mode == 'zeros' else torch.nn.ReplicationPad2d(pad_size)
        ...
        for dx, dy in zip(offsetsx, offsetsy):
            prod = feat_ref * feat_src[:, :, pad_top+dy : pad_top+dy+H, pad_l+dx : pad_l+dx+W]  # N, C, H, W
...
class FlowSamplingPoints:
    ...
    def __init__(self, steps=10, step_size=2):
        ...
        offsets = torch.linspace(-steps, steps, 2*steps+1, dtype=torch.int32) * step_size  # S = 2*steps+1
...
class DispSamplingPoints:
    ...
    def __init__(self, steps=40, step_size=1):
        # START TODO #################
        # A DispNet uses 1D correlation along horizontal epipolar lines to the left of the reference point.
        # Implement the sampling points accordingly. The final shape of self.offsets should be (steps+1, 2).
        # Hint: The offsets in x direction can be computed similarly as in the FlowSamplingPoints.
        offsets_x = torch.linspace(-steps, 0, steps+1, dtype=torch.int32) * step_size  # S = steps+1
...
class Corr:
    def __init__(self, steps=10, step_size=2, cuda_corr=False, corr_type="flow"):
        ...
        if not cuda_corr:
            ...
            self.corr_block = SpatialCorrelationSampler(kernel_size=1, patch_size=21, stride=1, padding=0,
                                                        dilation=1, dilation_patch=2)

        else:
            raise NotImplementedError("CUDA correlation layer for disparity correlation currently missing.")

        self.cuda_corr = cuda_corr
        self.num_sampling_points = (2*steps+1) ** 2 if corr_type == "flow" else steps+1  # TODO: return len(self.sampling_points) instead)
    ...
    @torch.no_grad()
    def visualize(self, image_ref, image_src, feat_ref, feat_src, num_points=1, x_refs=None, y_refs=None):
        ...
        src_vis = self._get_src_vis(image_src, corr, self.sampling_points, N, H, W, h, w, x_refs, y_refs)  # N, 3, H, W
    ...
    def _get_ref_vis(self, image_src, N, W, w, x_refs, y_refs):
        ...
        for n in range(N):
            ...
            markers_image = [{'xy_pos': (x_refs_orig[idx], y_refs_orig[idx]), 'desc': 'p_ref', 'desc_text_off': False}
    ...
    @torch.no_grad()
    def _get_src_vis(self, image_src, corr, sampling_points, N, H, W, h, w, x_refs, y_refs):
        ...
        for n in range(N):
            ...
            for idx in range(len(x_refs)):
                ...
                if not score_is_constant:  # TODO: what is this?
                    markers_image.append({'xy_pos': (scale * x[max_score_step], scale * y[max_score_step]), 'marker_color': (0, 255, 0), 'score': np.nan, 'marker_radius': 2})

                max_score = None
                for step in range(0, steps):
                    score_cur = score[step]
                    max_score = score_cur if ((max_score is None) or (score_cur > max_score)) else max_score
                ...
                for step in range(0, steps):
                    ...
                    markers_image.append(
                        {'xy_pos': (scale * x_cur, scale * y_cur), 'marker_color': (133, 153, 0), 'score': score_cur,
                         'desc_text_off': show_marker_text, 'score_text_off': show_marker_text})

            image_n_vis = np3d(image_n, markers=markers_image, marker_radius=4, marker_text_off=False,
                               marker_cmap='plasma', image_range_text_off=True, marker_range_text_off=False,
                               text_off=False,
                               out_format={'type': 'np', 'mode': 'RGB', 'channels': 'CHW', 'dtype': 'uint8'})
