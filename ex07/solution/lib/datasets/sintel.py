...
class SintelSequence:
    def __init__(self, root, name, renderpass='clean'):
        ...
        self.ids = frames
        self.id_to_idx = {int(frame): frame_idx for frame_idx, frame in enumerate(frames)}
        self.images = [SintelImage(osp.join(renderpass, name, "frame_{}.png".format(frame))) for frame in frames]
        self.depths = [SintelDepth(osp.join("depth", name, "frame_{}.dpt".format(frame))) for frame in frames]
        self.flows = [SintelFlow(osp.join("flow", name, "frame_{}.flo".format(frame))) for frame in frames]
        self.poses = [SintelPose(osp.join("camdata_left", name, "frame_{}.cam".format(frame))) for frame in frames]
        self.intrinsics = [SintelIntrinsics(osp.join("camdata_left", name, "frame_{}.cam".format(frame))) for frame in frames]

        assert len(self.images) == len(self.depths) == len(self.poses) == len(self.intrinsics)
...
class Sintel(Dataset):
    def __init__(self, sample_confs, renderpass='clean', sequences=None,
                 root=None, aug_fcts=None, to_torch=False, verbose=True):

        self.renderpass = renderpass
        root = root if root is not None else self._get_path("Sintel", "root")

        super().__init__(root=root, aug_fcts=aug_fcts, to_torch=to_torch, verbose=verbose, sample_confs=sample_confs,
                         sequences=sequences)

    def _init_samples(self, sample_confs, sequences=None):

        image_root = osp.join(self.root, self.renderpass)

        sequence_names = sorted(os.listdir(image_root))
        sequence_names = [s for s in sequence_names if s in sequences] if sequences is not None else sequence_names
        sequences = [SintelSequence(self.root, name, self.renderpass) for name in sequence_names]
                    ...
                    for key, conf in sample_conf.items():
                        ...
                        if isinstance(conf, tuple):
                            ...
                            if offset_num in sequence.id_to_idx:
                                ...
                                sample.data[key] = getattr(sequence, data_type)[offset_idx]
                            else:
                                sample_valid = False
                                break
                            ...
                            for conf in conf:
                                ...
                                if offset_num in sequence.id_to_idx:
                                    offset_idx = sequence.id_to_idx[offset_num]
                                    sample.data[key].append(getattr(sequence, data_type)[offset_idx])
                                else:
                                    sample_valid = False
                                    break
