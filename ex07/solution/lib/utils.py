...
def get_epipole(K, to_ref_transform, hom=False):
    ...
    return project_to_image(X_ref=X_ref, K=K, to_ref_transform=to_ref_transform, return_hom=hom)
    # END TODO ###################
...
def compute_fundamental_matrix(K, to_ref_transform):
    ...
    # 1. Compute epi_from_ref_in_cur_x using the functions get_epipole and cross_mat_from_vec.
    epi_from_ref_in_cur = get_epipole(K=K, to_ref_transform=to_ref_transform, hom=True)
...
def rectify_images(image_l, image_r, K, r_to_l_transform):
    ...
    # START TODO ###################
    T = l_to_r_translation = trans_from_transform(l_to_r_transform)
