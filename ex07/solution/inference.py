...
from lib.utils import load_model, load_all, rectify_images
...
def load_data(path, rectify=False):
    ...
    if rectify:
        ...
        image_left, image_right, H_l, H_r, right_to_left_transform = rectify_images(image_left, image_right, K,
...
def write_pred(sample, model_output, path):
    ...
    pred_disp = model_output['pred_disp'] if 'pred_disp' in model_output else model_output['pred_flow'][:, 0:1, :, :]
    pred_disp = np.array(Image.fromarray(pred_disp.squeeze()).resize((w_orig, h_orig), Image.NEAREST)).astype(np.float32)
    ...
    if 'H_l' in sample:
        import cv2
        H_l = sample['H_l']
        pred_disp = cv2.warpPerspective(pred_disp, np.linalg.inv(H_l), pred_disp.shape[::-1], flags=cv2.INTER_NEAREST)
...
def setup_args():
    ...
    parser.add_argument('--path', help="Path to folder with input images. Outputs will be written to same folder.")
    parser.add_argument('--rectify', action='store_true', help='Rectify the input images. Requires the relative pose and intrinsics.')
