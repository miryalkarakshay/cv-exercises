...
def log_tensor(writer, desc, tensor, step, idx=None, full_batch=False, colorize=True,
    ...
    if ch == 1:
        ...
        img = np3d(arr=tensor, channels='FLOW', text=text, gray=(not colorize),
                   ...
                   lower_clipping_thresh=lower_clipping_thresh,
                   mark_clipping=mark_clipping, clipping_color=clipping_color,
                   invalid_values=invalid_values, mark_invalid=mark_invalid, invalid_color=invalid_color,
                   markers=markers, marker_radius=marker_radius, marker_text_off=marker_text_off,
                   marker_cmap=marker_cmap,
                   ignore_marker_scores=ignore_marker_scores, min_marker_score=min_marker_score,
                   max_marker_score=max_marker_score,
                   image_range_text_off=image_range_text_off, marker_range_text_off=marker_range_text_off,
                   text_off=text_off,
                   out_format={'type': 'np', 'mode': 'RGB', 'channels': 'CHW', 'dtype': 'uint8'})
        ...
        raise ValueError("log_tensor was called with a tensor of shape {} but "
                         "can only be used with a NCHW tensor with C=1, 2 or 3.".format(tensor.shape))

    writer.add_image(desc, img, step)


def log_tensor_list(writer, desc, tensor_list, step, labels=None, idx=None, full_batch=False, every_nth=None,
                    colorize=True, clipping=False, upper_clipping_thresh=None, lower_clipping_thresh=None,
                    mark_clipping=False, clipping_color=None,
                    invalid_values=None, mark_invalid=False, invalid_color=None,
                    text=None, cmap=None,
                    markers=None, marker_radius=None, marker_text_off=False, marker_cmap=None,
    ...
    for tensor_idx, tensor in enumerate(tensor_list):
        ...
        label = (idx_label + "/" + str(label) if not isinstance(label, str) else idx_label + label) if idx_label is not None else label

        log_tensor(writer=writer, desc=desc, tensor=tensor, step=step, idx=label, full_batch=full_batch,
                   colorize=colorize,
                   clipping=clipping, upper_clipping_thresh=upper_clipping_thresh,
                   lower_clipping_thresh=lower_clipping_thresh,
                   mark_clipping=mark_clipping, clipping_color=clipping_color,
...
class Logger:
    ...
    def log_in_data(self, sample, step):
        ...
        log_tensor(self.writer, "{}/{}/00_images/ref".format(self.overview_base, base_name), images[0], step,
                   full_batch=full_batch)

        log_tensor_list(self.writer, base_name + "/00a_images_orig", images_orig, step, full_batch=full_batch)
        log_tensor_list(self.writer, base_name + "/00b_images_spatial", images_spatial, step, full_batch=full_batch)
        log_tensor_list(self.writer, base_name + "/00c_images_in", images, step, full_batch=full_batch)

        if gt_flow is not None:
            log_tensor(self.writer, base_name + "/01_gt_flow", gt_flow, step, full_batch=full_batch)
            log_tensor(self.writer, "{}/{}/01_gt_flow".format(self.overview_base, base_name), gt_flow, step,
                       full_batch=full_batch)

        if gt_disp is not None:
            log_tensor(self.writer, base_name + "/02_gt_disp", gt_disp, step, full_batch=full_batch)
            log_tensor(self.writer, "{}/{}/02_gt_disp".format(self.overview_base, base_name), gt_disp, step,
                       full_batch=full_batch)
    ...
    def log_model(self, sample, step):
        ...
        model_vis = model.visualize(sample)

        for model_vis_name, vis in model_vis.items():
            log_tensor(self.writer, "{}/{}".format(base_name, model_vis_name), vis, step, full_batch=full_batch)
    ...
    def log_pred(self, model_output, step):
        ...
        pred_flows_all = model_output['pred_flows_all'] if 'pred_flows_all' in model_output else None

        pred_disp = model_output['pred_disp'] if 'pred_disp' in model_output else None
        pred_disps_all = model_output['pred_disps_all'] if 'pred_disps_all' in model_output else None

        if pred_flow is not None:
            log_tensor(self.writer, base_name + "/01a_pred_flow", pred_flow, step, full_batch=full_batch)
            log_tensor(self.writer, "{}/{}/01_pred_flow".format(self.overview_base, base_name), pred_flow, step,
                       full_batch=full_batch)

        if pred_flows_all is not None:
            for level, pred_flow_ in enumerate(pred_flows_all):
                log_tensor(self.writer, base_name + "/01b_pred_flows_all/level_%d" % level, pred_flow_, step,
                           full_batch=full_batch)
        ...
        if pred_disps_all is not None:
            for level, pred_disp_ in enumerate(pred_disps_all):
                log_tensor(self.writer, base_name + "/02b_pred_disps_all/level_%d" % level, pred_disp_,
    ...
    def log_loss(self, loss, sub_losses, pointwise_losses, step, scalars_only):
        ...
        log_scalar(self.writer, "{}/{}/0_total_loss".format(self.overview_base, base_name), loss, step)
    ...
    def log_optim(self, step):
        ...
        for i, param_group in enumerate(optimizer.param_groups):
            lr = param_group['lr']
            log_scalar(self.writer, "{}/{}/lr/group_{}".format(self.overview_base, base_name, i), lr, step)
        ...
        for name, param in model.named_parameters():
            ...
            if param.grad is not None:
                log_histogram(self.writer, "{}/1_grads/{}".format(base_name, name), param.grad, step,
                              replace_NaNs=False)
    ...
    def log_metrics(self, metrics, qualitatives, step):
        ...
        for idx, (metric_name, metric_val) in enumerate(metrics.items()):
            log_scalar(self.writer, "{}/{}_{}".format(base_name, idx, metric_name), metric_val, step)
