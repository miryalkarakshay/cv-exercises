...
from .metrics import epe, pointwise_epe, photometric_loss, smoothness_loss
...
class DispLoss(nn.Module):
    ...
    def forward(self, sample, model_output):
        ...
        for level, pred_disp in enumerate(pred_disps_all):
            ...
            with torch.no_grad():
                gt_disp_resampled = F.interpolate(gt_disp, size=pred_disp.shape[-2:], mode=self.gt_interpolation,
                                                  align_corners=(False if self.gt_interpolation != 'nearest' else None))

            epe_loss = epe(gt=gt_disp_resampled, pred=pred_disp, weight=self.loss_weights[level])
            pointwise_epe_ = pointwise_epe(gt=gt_disp_resampled, pred=pred_disp, weight=self.loss_weights[level])
