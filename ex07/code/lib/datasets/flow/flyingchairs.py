...
class FlyingChairs(Dataset):
    def __init__(self, split=None, root=None, aug_fcts=None, to_torch=False, verbose=True):

        root = root if root is not None else self._get_path("FlyingChairs", "root")

        super().__init__(root=root, aug_fcts=aug_fcts, to_torch=to_torch, verbose=verbose, split=split)
    ...
    def _init_samples(self, split=None):
        ...
        split_list = np.loadtxt(osp.join(self.root, 'FlyingChairs_train_val.txt'), dtype=np.int32)
        for i in range(len(flows)):
            xid = split_list[i]
            if (split is None) or (split == 'training' and xid == 1) or (split == 'validation' and xid == 2):
                sample = FlyingChairsSample(name=flows[i][:5], images=[images[2*i], images[2*i+1]], flow=flows[i])
...
class FlyingChairsTrain(FlyingChairs):
    ...
    def __init__(self, root=None, aug_fcts=None, to_torch=False, verbose=True):
        ...
        split = "training"
        super().__init__(split=split, root=root, aug_fcts=aug_fcts, to_torch=to_torch, verbose=verbose)
