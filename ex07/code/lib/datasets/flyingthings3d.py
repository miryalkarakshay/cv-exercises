...
class FlyingThings3D(Dataset):
    def __init__(self, sample_confs, split, use_subset=False, root=None, aug_fcts=None, to_torch=False, verbose=True):

        self.split = split
        root = root if root is not None else self._get_path("FlyingThings3D", self.split, "root")

        super().__init__(root=root, aug_fcts=aug_fcts, to_torch=to_torch, verbose=verbose,
                         sample_confs=sample_confs, use_subset=use_subset)
                    ...
                    if isinstance(conf, DataConf):
                        ...
                        for file in os.listdir(osp.join(sequence, conf.path)):
                            ...
                            for filtered_sample in filtered_samples:
                                if filtered_sample[0] in sequence and filtered_sample[1] in file:
                                    exclude_file = True
