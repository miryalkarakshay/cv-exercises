...
from .blocks import deconv, conv
...
class FlowNet(nn.Module):
    def __init__(self, C=64, corr=True, cuda_corr=False):
        ...
        self.pred_1 = pred_block(C_curr + C_curr + 2)  # input: [deconv_1, conv5_1, interp(pred_0)]

        C_last = C_curr  # 512
        C_curr = C_curr // 2  # 256
        self.deconv_2 = deconv(2 * C_last + 2, C_curr)  # input: [deconv_1, conv5_1, interp(pred_0)]
        self.pred_2 = pred_block(C_curr + C_last + 2)  # input: [deconv2, conv4_1, interp(pred_1)]

        C_last = C_curr  # 256
        C_curr = C_curr // 2  # 128
        self.deconv_3 = deconv(3 * C_last + 2, C_curr)  # input: [deconv2, conv4_1, interp(pred_1)]
        self.pred_3 = pred_block(C_curr + C_last + 2)  # input: [deconv3, conv3_1, interp(pred_2)]

        C_last = C_curr  # 128
        C_curr = C_curr // 2  # 64
        self.deconv_4 = deconv(3 * C_last + 2, C_curr)  # input: [deconv3, conv3_1, interp(pred_2)]
        self.pred_4 = pred_block(C_curr + C_last + 2)  # input: [deconv4, conv2a, interp(pred_3)]
    ...
    def forward(self, image_1, image_2):
        ...
        pred = F.interpolate(pred, size=deconv_1.shape[-2:], mode='bilinear', align_corners=False).detach()
        rfeat1 = torch.cat((conv5_1, deconv_1, pred), 1)
        pred = self.pred_1(rfeat1)
        preds['pred_flows_all'].append(pred)

        deconv_2 = self.deconv_2(rfeat1)
        pred = F.interpolate(pred, size=deconv_2.shape[-2:], mode='bilinear', align_corners=False).detach()
        rfeat2 = torch.cat((conv4_1, deconv_2, pred), 1)
        pred = self.pred_2(rfeat2)
        preds['pred_flows_all'].append(pred)

        deconv_3 = self.deconv_3(rfeat2)
        pred = F.interpolate(pred, size=deconv_3.shape[-2:], mode='bilinear', align_corners=False).detach()
        rfeat3 = torch.cat((conv3_1, deconv_3, pred), 1)
        pred = self.pred_3(rfeat3)
        preds['pred_flows_all'].append(pred)

        deconv_4 = self.deconv_4(rfeat3)
        pred = F.interpolate(pred, size=deconv_4.shape[-2:], mode='bilinear', align_corners=False).detach()
        rfeat4 = torch.cat((conv2a, deconv_4, pred), 1)
        pred = self.pred_4(rfeat4)
        preds['pred_flows_all'].append(pred)

        deconv_5 = self.deconv_5(rfeat4)
        pred = F.interpolate(pred, size=deconv_5.shape[-2:], mode='bilinear', align_corners=False).detach()
    ...
    def visualize_corr(self, image_left, image_right, x_refs=None, y_refs=None):
        ...
        return self.corr_block.visualize(image_left, image_right, conv3a, conv3b, x_refs=x_refs, y_refs=y_refs)
