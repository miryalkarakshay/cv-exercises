...
class FlowNetAugmentation:
    def __init__(self):
        ...
        self.color_aug = ColorJitter(saturation=(0, 2), contrast=(0.01, 8), brightness=(0.01, 2.0), hue=0.5)
    ...
    def __call__(self, sample):
        ...
        gt_disp = sample['gt_disp'] if ('gt_disp' in sample and not self.augment_image_only) else None  # H, W
        gt_flow = sample['gt_flow'] if ('gt_flow' in sample and not self.augment_image_only) else None  # 2, H, W
        ...
        images_spatial, gt_flow, gt_disp, spatial_params = self.spatial_transform(images, gt_flow, gt_disp)
        ...
        images_spatial = [np.transpose(image_spatial, [2, 0, 1]) for image_spatial in images_spatial]  # 3, H, W
    ...
    def color_transform(self, images):
        ...
        if np.random.rand() < self.color_aug_prob:
            ...
            if np.random.rand() < self.asymmetric_color_aug_prob:
                images = [np.array(self.color_aug(Image.fromarray(image)), dtype=np.uint8) for image in images]
    ...
    def spatial_transform(self, images, flow, disp):
        ...
        if np.random.rand() < self.spatial_aug_prob:
            ...
            if images is not None and len(images) > 0 and any([image is not None for image in images]):
                images = [cv2.resize(image, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR) if image is not None else None for image in images]
            ...
            if disp is not None:
                disp = cv2.resize(disp, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_NEAREST)
                disp = disp * scale_x
                if sht is None or swd is None:
            ...
            if images is not None and len(images) > 0 and any([image is not None for image in images]):
                images = [image[y0:y0 + cht, x0:x0 + cwd] if image is not None else None for image in images]
            ...
            if images is not None and len(images) > 0 and any([image is not None for image in images]):
                images = [cv2.resize(image, (cwd, cht), interpolation=cv2.INTER_LINEAR) if image is not None else None for image in images]
